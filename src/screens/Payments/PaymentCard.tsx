...
const PaymentCard: React.FC<PaymentCardProps> = ({ title, price, benefits }) => {
const navigate = useNavigate()
  ...
  return (
    <div className="bg-white rounded-lg w-68 shadow p-6 mb-4">
      <h2 className="text-xl text-bizBlue font-bold mb-2">{title}</h2>
      <p className=" text-black text-sm font-bold mb-4">Kshs{` ${price}`} per month</p>
      ...
      {
        price === 0 ?
<button onClick={ handleSubscriptionType} className="w-full bg-bizBlue text-white py-2 px-2 rounded-lg mt-4">
